...
```bash
cd /home/ra/Projects/eye-tracking
uv sync
uv run eye-tracking download-model
uv run eye-tracking run
```

Python 3.12 is selected in `.python-version`. Setup downloads dependencies and the
version 1 Face Landmarker model; tracking runs locally afterward. A desktop session
is needed for the OpenCV preview. Press **Q** or **Esc** to exit, or **Ctrl-C** in the terminal.

```bash
# Select a different camera and record measurements (no video is saved).
uv run eye-tracking run --camera 5 --output outputs/session-001.jsonl
...
`just setup`, `just run`, `just test`, and `just lint` wrap the common commands.
...
```bash
uv run eye-tracking web
# Open http://127.0.0.1:8080 and press Start camera.
```
...
`just web` launches the same dashboard. Development checks require Node.js as well
as uv to compare browser measurements with Python.
